...
| **marketing-surfaces** | Ship the public side of a user-visible feature, one verified step per surface: docs/help page, blog or changelog post, `mockups` screenshots, pricing tier line, landing copy, local preview, email, social, in-app instructional copy. Non-technical by default; technical only for developer-reference pages (API/MCP docs), which the launch post links with a "send this to your engineer" hand-off. Triaged on every change (best-practices finishing sequence, bug-fix, PDD audit); drafts are reviewed inline before any PR, and marketing PRs are separate from the feature PR (changelog excepted). |
...
| **change-log** | Log each feature or fix in `CHANGELOG.md` using plain, business-user-friendly language. Powers the product updates blog. The only marketing artifact that stays in the feature PR. |
...
| **update-docs** | Sync maintainer documentation after completing a feature. Finds stale content across CLAUDE.md, README, in-repo guides, plan files, sibling repos, public skills/plugins, and downstream project docs. Customer-facing pages are `marketing-surfaces`' job, not this one's. |
