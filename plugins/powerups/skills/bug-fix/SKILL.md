...
Run the finishing sequence from `powerups:best-practices` practice #9: `/simplify`, `powerups:change-log` (skip if the fix isn't user-facing), `update-docs` (maintainer docs), `powerups:marketing-surfaces` (customer-facing surfaces — it owns the triage; report its output), and lint. The full test suite already ran in Step 7.
