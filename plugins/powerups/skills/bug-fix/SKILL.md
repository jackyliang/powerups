...
Run the finishing sequence from `powerups:best-practices` practice #9: `/simplify`, `powerups:change-log` (skip if the fix isn't user-facing), `update-docs`, `powerups:marketing-surfaces` triage (does a help article, docs page, or post describe the old behavior? if unsure, ask; any update is drafted inline and lands as a separate PR), and lint. The full test suite already ran in Step 7.
