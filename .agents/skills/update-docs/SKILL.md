...
---
<!-- Generated by scripts/sync-devin-skills.py from plugins/powerups/skills/update-docs/SKILL.md. Do not edit. -->
...
**Scope: docs written for whoever maintains the code.** CLAUDE.md, README, `docs/`, `.env.example`, `plans/`, and the same files in sibling repos. Anything a customer reads — help/docs pages, blog or product-updates posts, pricing, screenshots, in-app copy — is `marketing-surfaces`; `CHANGELOG.md` is `change-log`. This skill never touches those, and never duplicates their triage.
...
If a sibling repo is the public/marketing site, scan only its maintainer docs (CLAUDE.md, README). Its published pages are out of scope here — the finishing sequence runs `marketing-surfaces` right after this skill, and that triage covers them.
...
- **Stay in scope.** Customer-facing pages are `marketing-surfaces`; `CHANGELOG.md` is `change-log`. Don't re-triage them here.
