...
2. **`change-log`** — the `CHANGELOG.md` entry. User-facing changes only; stays in this PR.
3. **`update-docs`** — maintainer docs: CLAUDE.md, README, guides, sibling repos. Small changes cause drift too.
4. **`marketing-surfaces`** — customer-facing surfaces, triaged on every change including bug fixes. Usually N/A, said in one line per surface. Anything it does update lands as its own PR, never in this one.
