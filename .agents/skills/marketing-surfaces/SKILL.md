...
---
<!-- Generated by scripts/sync-devin-skills.py from plugins/powerups/skills/marketing-surfaces/SKILL.md. Do not edit. -->
...
- A user-visible feature shipped (invoked by `plan-driven-development` — its last milestone and post-completion checklist)
...
Default: **write for a non-technical reader** — a support lead or a store owner, not an engineer. Run `simple-design-principles` over every text surface and treat it as a gate, not a suggestion:
